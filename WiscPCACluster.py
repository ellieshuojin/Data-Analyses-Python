...
# applying PCA
pca = PCA()  # creating a PCA transformation object for data reduction
wiscPCs = pca.fit_transform(wiscFeatures) # fit the data

# scree plot, and then looking for where an "elbow" occcurs
plt.plot(np.arange(1,len(featureNames)+1), pca.explained_variance_ratio_)
plt.xlabel('Component number')
plt.ylabel('Explained variance')
plt.show()

## using the first two components (based on where "elbow" occurred)
PC = wiscPCs[:,:2]
